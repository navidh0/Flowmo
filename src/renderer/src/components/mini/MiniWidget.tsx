 ...
 *
 * Frameless also means no title-bar close button, so the widget carries its own small X at the
 * end of the top row, set apart from the two controls so it can't be mistaken for Dismiss. It
 * goes through `app.setMiniWidget(false)`, the same path as the tray's toggle: that counts as
 * the user's explicit choice, so an auto-opened widget isn't reopened until the window next
 * leaves. It sits in normal flow inside its own no-drag box, like Play and Skip, at least
 * 8px from any edge. 0.4.2 and 0.5.0 absolutely positioned it 4px from the corner, where the
 * window's resize border and drag region begin, and on Windows a real click never reached it
 * (e2e now clicks it with the real OS mouse, see osClickInMini).
 */
    ...
    >
      ...
      <div className="flex items-center gap-1.5 px-2.5 pt-2">
        ...
        <span className="ml-auto flex shrink-0 items-center gap-1.5">
          ...
          <span className="flex" style={NO_DRAG}>
            <button
              type="button"
              aria-label="Close widget"
              title="Close the widget"
              onClick={() => void window.flowdo.app.setMiniWidget(false)}
              className="grid h-5 w-5 place-items-center rounded text-[var(--color-text-muted)] opacity-70 outline-none transition-opacity hover:bg-[var(--color-surface-raised)] hover:opacity-100 focus-visible:opacity-100 focus-visible:ring-1 focus-visible:ring-[var(--color-text-muted)]"
            >
